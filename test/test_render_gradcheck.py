...
if grid.use_background:
    ...
    grid.background_data.data[..., :-1] = (
        torch.randn_like(grid.background_data.data[..., :-1]) * 0.01
    )
...
if ENABLE_TORCH_CHECK:
    ...
    with Timing("torch_backward"):
        s.backward()
    grid_sh_grad_t = (
        grid.sh_data.grad.clone().cpu()
        if grid.sh_data.grad is not None
        else torch.zeros_like(grid_sh_grad_s)
    )
    grid_density_grad_t = (
        grid.density_data.grad.clone().cpu()
        if grid.density_data.grad is not None
        else torch.zeros_like(grid_density_grad_s)
    )
    if grid.basis_type == svox2.BASIS_TYPE_3D_TEXTURE:
        grid_basis_grad_t = grid.basis_data.grad.clone().cpu()
    if grid.use_background:
        grid_bg_grad_t = (
            grid.background_data.grad.clone().cpu()
            if grid.background_data.grad is not None
            else torch.zeros_like(grid_bg_grad_s)
        )
