...
parser.add_argument(
    "--train", action="store_true", default=False, help="render train set"
)
parser.add_argument(
    "--render_path",
    action="store_true",
...
parser.add_argument(
    "--no_lpips", action="store_true", default=False, help="Disable LPIPS (faster load)"
)
parser.add_argument(
    "--no_vid", action="store_true", default=False, help="Disable video generation"
)
parser.add_argument(
    "--no_imsave",
    action="store_true",
...
# Camera adjustment
parser.add_argument(
    "--crop", type=float, default=1.0, help="Crop (0, 1], 1.0 = full image"
)
...
parser.add_argument(
    "--ray_len", action="store_true", default=False, help="Render the ray lengths"
)
...
render_dir = path.join(
    path.dirname(args.ckpt), "train_renders" if args.train else "test_renders"
)
want_metrics = True
if args.render_path:
...
with torch.no_grad():
    ...
    n_images_gen = 0
    c2ws = (
        dset.render_c2w.to(device=device)
        if args.render_path
        else dset.c2w.to(device=device)
    )
    # DEBUGGING
    ...
    if not args.no_vid and len(frames):
        ...
        imageio.mimwrite(
            vid_path, frames, fps=args.fps, macro_block_size=8
