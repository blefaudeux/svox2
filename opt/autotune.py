...
def run_exp(
    ...
    opt_base_cmd += ["-t", train_dir, data_dir]
    if config != "":
    ...
    if eval_mode and path.isfile(ckpt_path):
        ...
        try:
            ...
            print(
                "Error occurred while running OPT for exp",
                train_dir,
                "on",
    ...
    if eval_mode:
        eval_base_cmd = ["python", "render_imgs.py", ckpt_path, data_dir]
        if config != "":
        ...
        if not path.exists(psnr_file_path):
            ...
            print(eval_cmd)
            try:
                eval_ret = subprocess.check_output(
                    eval_cmd, shell=True, env=env
                ).decode(sys.stdout.encoding)
            except subprocess.CalledProcessError:
                print(
                    "Error occurred while running EVAL for exp",
                    train_dir,
                    "on",
                    env["CUDA_VISIBLE_DEVICES"],
                )
        ...
        if enable_render:
            ...
            try:
                render_ret = subprocess.check_output(
                    render_cmd, shell=True, env=env
                ).decode(sys.stdout.encoding)
            except subprocess.CalledProcessError:
                print(
                    "Error occurred while running RENDER for exp",
                    train_dir,
                    "on",
                    env["CUDA_VISIBLE_DEVICES"],
                )
        ...
        test_stats = [
            eval(x.split("eval stats:")[-1].strip())
            for x in opt_ret.split("\n")
            if x.startswith("eval stats: ")
        ]
        if len(test_stats) == 0:
            ...
            test_psnrs = [
                stats["psnr"] for stats in test_stats if "psnr" in stats.keys()
            ]
...
def create_prodvars(variables, noise_stds={}):
    ...
    def auto_list(x):
        if isinstance(x, list):
            ...
            raise NotImplementedError(
                "variable value must be list of values, or str generator"
            )
    ...
    variables = [
        [(i, val) for val in variables[varname]] for i, varname in enumerate(varnames)
    ]
    prodvars = list(itertools.product(*variables))
    noise_vals = np.random.randn(len(prodvars), len(varnames)) * noise_stds
    prodvars = [
        {
            varnames[i]: ((val + n) if n != 0.0 else val)
            for (i, val), n in zip(sample, noise_vals_samp)
        }
        for sample, noise_vals_samp in zip(prodvars, noise_vals)
    ]
    return prodvars
...
if __name__ == "__main__":
    ...
    leaderboard_path = path.join(
        train_root, "results.txt" if args.eval else "leaderboard.txt"
    )
    ...
    for i, gpu in enumerate(args.gpus):
        process = Process(
            target=process_main, args=(gpu, args.eval, args.render, pqueue)
        )
        process.daemon = True
        process.start()
    ...
    if args.eval:
        ...
        with open(leaderboard_path, "w") as leaderboard_file:
            ...
            if len(all_psnr):
                lines.append(
                    "Average PSNR: " + str(sum(all_psnr) / len(all_psnr)) + "\n"
                )
            if len(all_ssim):
                lines.append(
                    "Average SSIM: " + str(sum(all_ssim) / len(all_ssim)) + "\n"
                )
            if len(all_lpips):
                lines.append(
                    "Average LPIPS: " + str(sum(all_lpips) / len(all_lpips)) + "\n"
                )
            if len(all_times):
                lines.append(
                    "Average Time (mins): "
