    ...
    for intImage, objImage in enumerate(
        ...
        if objIntrinsics.model == "SIMPLE_RADIAL":
            objCameras[objImage.name] = {
                ...
                "intPoints": [
                    intPoint for intPoint in objImage.point3D_ids if intPoint != -1
                ],
            }
...
def generate_masks(vid_root, args, overwrite=False):
    ...
    for file_ind, file in enumerate(tqdm(files, desc=f"masks: {vid_name}")):
        ...
        obj_predictions = maskrnn_model(
            [
                torch.FloatTensor(im.transpose(2, 0, 1) / 255.0)[[2, 0, 1], :, :].to(
                    device
                )
            ]
        )[0]
...
def run_colmap(vid_root, args, factor, overwrite=False):
    ...
    if args.known_intrin:
        ...
        if os.path.isfile(intrin_path):
            ...
            if args.noradial:
                extractor_cmd += (
                    f' --ImageReader.camera_params "{focal:.10f},{cx:.10f},{cy:.10f}"'
                )
            else:
...
def render_movie(vid_root, args):
    ...
    obj_cameras, obj_points = read_colmap(os.path.join(vid_root, "sparse", "0"))
    ...
    for file_idx, file in enumerate(tqdm(files, desc=f"render: {vid_name}")):
        ...
        if fn in obj_cameras:
            ...
            for obj_point in [
                obj_points[int_point] for int_point in obj_camera["intPoints"]
            ]:
    ...
    moviepy.editor.ImageSequenceClip(sequence=debug_dir, fps=25).write_videofile(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--colmap-root",
        type=str,
        default="/home/sxyu/builds/colmap",
        help="COLMAP installation dir (only needed for vocab tree in case of sequential matcher)",
    )
    parser.add_argument(
        "--image-input", default="raw", help="location for source images"
    )
    parser.add_argument(
        "--mask-output", default="masks", help="location to store motion masks"
    )
    parser.add_argument(
        "--known-intrin",
        action="store_true",
    ...
    parser.add_argument(
        "--debug", action="store_true", default=False, help="render debug video"
    )
    parser.add_argument(
        "--noradial",
        action="store_true",
    ...
    parser.add_argument(
        "--use-masks", action="store_true", default=False, help="use automatic masks"
    )
    parser.add_argument(
        "--images-resized",
