...
parser.add_argument(
    "--every", type=int, default=16, help="Every x images used for testing"
)
parser.add_argument(
    "--dry_run",
    action="store_true",
    help="Dry run, prints renames without modifying any files",
)
parser.add_argument("--yes", "-y", action="store_true", help="Answer yes")
parser.add_argument(
    "--random",
    action="store_true",
    help="If set, chooses the split randomly rather than at a fixed interval "
...
if do_proceed:
    ...
    base_files_map = {
        x: f"{int(i % args.every == 0)}_" + x for i, x in enumerate(base_files)
    }
